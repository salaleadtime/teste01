...
function renderVoteGrid(participants, revealed) {
  ...
  participants.filter((p) => p.role !== 'master').forEach((p) => {
    ...
    if (p.role === 'observer') {
      ...
      pill = `<span class="vs-pill ${p.role === 'qa' ? 'val-qa' : 'val-dev'}">${p.vote}</span>`;
    } else if (p.hasVoted) {
...
function renderSplitResults(participants) {
  ...
  const devRows = devs.map((p) => {
    ...
    const chip = p.vote
      ? `<span class="vote-chip developer ${isMod ? 'vote-winner' : ''}"><span class="chip-points">${p.vote}</span>${hours ? `<span class="chip-hours">${hours}</span>` : ''}</span>`
